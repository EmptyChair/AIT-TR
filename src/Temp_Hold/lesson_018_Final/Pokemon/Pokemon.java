package Temp_Hold.lesson_018_Final.Pokemon;
...
public class Pokemon {
    ...
    private static int additionalHp;

    static {
        Scanner s1 = new Scanner(System.in);
        System.out.println("Please provide additional HP which will be applied to all pokemons");
        additionalHp = s1.nextInt();
        System.out.println("Thanks! All pokemon are now beefier by "+additionalHp+" HP.");
    }
    ...
    Pokemon() {
        ...
        setHp(35+additionalHp);
        setCurrentHp(35+additionalHp);
        setAttack(55);
        setDefence(40);
        setSpeed(90);
    ...
    Pokemon(String name, String pokemonType, int hp, int currentHp, int attack, int defence, int speed, String specialAttack, String specialDefence, boolean asleep) {
        ...
        setCurrentHp(currentHp+additionalHp);
        setAttack(attack);
        setDefence(defence);
        setSpeed(speed);
    ...
    public Pokemon[] attack(Pokemon[] team) {
        int damage;
        for (int i = 0; i<team.length; i++) {
            //the first member of the opposing team not to be asleep gets whacked, then the loop stops
            if (!team[i].isAsleep()) {
                System.out.println(getName() + " (" + getPokemonType() + ") strikes at " + team[i].getName() + " (" + team[i].getPokemonType() + ") with all its might!");
                // calculate damage, if any
                if (getAttack() > team[i].getDefence()) {
                    damage = getAttack() - team[i].getDefence();
                    team[i].setCurrentHp(team[i].getCurrentHp()-damage);
                    System.out.println(getName() + " deals " + (getAttack() - team[i].getDefence()) + " HP of damage!!!");
                    //if the whacked pokemon has fallen below 1 Hp, put him to sleep
                    if (team[i].getCurrentHp()<1) {
                        team[i].setAsleep(true);
                        System.out.println(team[i].getName()+" ("+team[i].getPokemonType()+") has fallen asleep! It might recover yet, but its part in this round is over...");
                    }
                } else {
                    System.out.println("The attack fails miserably, inflicting no damage!");
                    System.out.println(getName() + " is humiliated!");
                    System.out.println(team[i].getName() + " is making mocking noises!");
                }
